        ...
        <div class="main-l">
            ...
            <div class="main-m">
                <script type="text/javascript">
                  document.getElementById('info').innerHTML = '<h2>VISUALIZAR - QUESTÃO(ÕES) - EDITAR</h2>';
                </script>
...
<div class="main-one-columns">
  <div class="main-one-columns-one">
    ...
    <button class="a b" type="submit" onclick="atualizar_questao()" style="margin: 0cm; padding:0.5cm">Salvar alteração</button>
    <br>
...
System.Ajax.Get({
  url:"../../../../api.php", 
  data:"x=buscar_questao_id&id="+data.id,
  success:function(json){

    var rtn = JSON.parse(json);
 
    document.getElementById('pergunta').value = rtn[0]['pergunta'];
    document.getElementById('numero').value = rtn[0]['numero'];
    document.getElementById('imagem_id').value = rtn[0]['imagem'];

    var curso = rtn[0].curso;
    var prova = rtn[0].prova;
        ...
        data: "x=buscar_cursos",
          success:function(json){
            ...
            for (var i = rtn.length - 1; i >= 0; i--) {
              if (curso == rtn[i]['id']) {
               html = html + '<option value="'+rtn[i]['id']+'" selected>'+rtn[i]['nome']+'</option>';
              } else {
  ...
  function atualizar_questao() {
    ...
    if (pergunta !== "") {
     ...
     if (numero >= 1) {
      ...
      System.Ajax.Get({
          url: "../../../../api.php",
          data: "x=atualizar_questao&pergunta="+encodeURIComponent(pergunta)+"&curso="+curso_id+"&numero="+numero+"&prova="+prova_id+"&imagem="+imagem_id+"&id="+data.id,
